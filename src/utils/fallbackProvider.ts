...
import { BaseProvider, isCommunityResource } from "@ethersproject/providers";
...
import { allSettledResults } from "./general";

/*
 * The FallbackProvider in this file is a fork of ethers, but it fixes a
 * bug (issue #2030 on GH's repo at ethers-io/ethers.js) where the provider
 * may potentially hang. This fixes the issue by, instead of using Promise.all
 * on network checks, using Promise.allSettled.
 */
...
const checkNetworks = (networks: Array<Network>): Network | null => {
  let result = null;
...
const median = (values: Array<number>, maxDelta?: number): number | null => {
...
function exposeDebugConfig(config: RunningConfig, now?: number): any {
  ...
  if (now) {
    result.duration = now - (config.start ?? 0);
  }
  if (config.done) {
    if (config.error) {
...
function getProcessFunc(
  ...
  switch (method) {
    ...
    case "getEtherPrice":
      ...
      return function (configs: Array<RunningConfig>): number {
        return median(configs.map((c) => c.result)) ?? 0;
      };
    ...
    () => {
      return new Promise((resolve) => {
        setTimeout(function () {
          ...
          if (config.cancelled) {
            // @ts-ignore We're done; just quit
            return resolve(null);
          }

          // @ts-ignore Try again, next block
...
async function getRunner(
  ...
  throw new Error("unknown method error");
}
...
export class FallbackProvider extends BaseProvider {
  ...
  constructor(
    ...
    const providerConfigs: Array<FallbackProviderConfig> = providers.map(
      (configOrProvider) => {
        ...
        if (weight % 1 || weight > 512 || weight < 1) {
          throw new Error("invalid weight; must be integer in [1, 512]");
        }
    ...
    if (quorum == null) {
      ...
      throw new Error("quorum will always fail; larger than total weight");
    }

    // Are all providers' networks are known
    let networkOrReady: Network | null | Promise<Network> = checkNetworks(
  ...
  async detectNetwork(): Promise<Network> {
    const networks = await allSettledResults(
  ...
  async perform(method: string, params: { [name: string]: any }): Promise<any> {
    ...
    if (method === "sendTransaction") {
      const results: Array<string | Error> = await allSettledResults(
    ...
    while (true) {
      ...
      let inflightWeight = configs
        .filter((c) => c.runner && t0 - (c.start ?? 0) < (c.stallTimeout ?? 0))
        .reduce((accum, c) => accum + (c.weight ?? 0), 0);
      ...
      while (inflightWeight < this.quorum && i < configs.length) {
        ...
        config.start = now();
        config.staller = stall(config.stallTimeout ?? 0);
        config.staller.wait(() => {
          config.staller = undefined;
        });
        ...
        if (this.listenerCount("debug")) {
          this.emit("debug", {
            action: "request",
            rid: rid,
            backend: exposeDebugConfig(config),
            request: { method: method, params: deepCopy(params) },
            provider: this,
          });
        }

        inflightWeight += config.weight ?? 0;
      }
    ...
    throw new Error("failed to meet quorum");
  }
}
